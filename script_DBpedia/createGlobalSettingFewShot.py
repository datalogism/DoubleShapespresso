...
def getRevelantPropDB(idEnt):
    query='''select ?p (COUNT(distinct ?a) as ?nb) where {?a a <'''+str(idEnt)+'''>. ?a ?p ?o FILTER (!regex(?p, "http://dbpedia.org/property/.*"))} GROUP BY ?p ORDER BY DESC (?nb) '''
    print(query)
    sparql = SPARQLWrapper(sparql_dbpedia)
    sparql.setQuery(query)
    sparql.setReturnFormat(JSON)
...
"""
    ...
    #print(qres)
    #return qres
    return [ str(row["subject"]["value"])+" "+idProp+" "+str(row["o1"]["value"]) for row in qres["results"]["bindings"]]
...
def get_DTropertiesRealised(idClass,idProp):
    ...
    if(len(qres["results"]["bindings"][0].keys())==0):
        ...
        return [ str(row["datatype"]["value"]) for row in qres["results"]["bindings"]]
...
def get_shexj(idProp,shape):
    ...
    if("http://www.w3.org/ns/shacl#datatype" in data.keys()):
        ...
        response["triple_constraint"]["valueExpr"]={"type": "NodeConstraint","datatype":data["http://www.w3.org/ns/shacl#datatype"]}
   
    return response

def get_microJsonLD(idProp,shape):
    query="""
    PREFIX sh: <http://www.w3.org/ns/shacl#> 
    PREFIX owl: <http://www.w3.org/2002/07/owl#> 
    PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> 
...
n_shot=3
idx=0
exluded_prop=["http://dbpedia.org/ontology/abstract","http://dbpedia.org/ontology/thumbnail","http://dbpedia.org/ontology/wikiPageExternalLink","http://dbpedia.org/ontology/wikiPageID","http://dbpedia.org/ontology/wikiPageLength","http://dbpedia.org/ontology/wikiPageRevisionID","http://dbpedia.org/ontology/wikiPageWikiLink","http://purl.org/dc/terms/subject","http://purl.org/linguistics/gold/hypernym","http://www.w3.org/2000/01/rdf-schema#comment","http://www.w3.org/2000/01/rdf-schema#label","http://www.w3.org/2002/07/owl#sameAs","http://www.w3.org/ns/prov#wasDerivedFrom","http://xmlns.com/foaf/0.1/depiction","http://xmlns.com/foaf/0.1/isPrimaryTopicOf","http://www.w3.org/2000/01/rdf-schema#seeAlso","http://www.w3.org/2002/07/owl#differentFrom","http://dbpedia.org/ontology/wikiPageInterLanguageLink","http://dbpedia.org/ontology/wikiPageRedirects","http://schema.org/sameAs"]
for prop in relevant_prop.keys():
    ...
    if(prop not in exluded_prop): 
        ...
        for card in distrib_dict.keys():
            ...
            if(ratio_card>0.1):
                card_text=card_text+" "+str(ratio_card)+"% instances in the class have "+card+"object(s) when using the property,\n"
        ...
        constr_=get_microJsonLD("http://dbpedia.org/ontology/residence",shape)
        doc.add("answer_"+str(idx)+".shaclj",str(constr_))
        idx=idx+1


with open("/user/cringwal/home/Desktop/KCLvisit/shapespresso-main/resources/dbpedia_global_few_shot_examples.toml", 'w') as toml_file:
